 ...
 *   run /tools/blackbox.js --debug 1
 *   run /tools/blackbox.js --debug 2
 * 
 * FEATURES:
 *   🎯 29 types de contrats supportés
 *   🔄 Scan automatique tous les 30s
 *   💾 Historique dans /state/contracts-history.json
 *   🎉 Toast par contrat résolu
 *   📊 Statistiques cumulées
 * 
 * CONTRATS SUPPORTÉS:
 *   - Algorithmes: Prime Factor, Subarray Sum, Stock Trader I-IV
 *   - Graphes: Shortest Path, Proper Coloring, Array Jumping
 *   - DP: Unique Paths, Triangle Path, Ways to Sum
 *   - Compression: RLE, LZ Compression/Decompression
 *   - Encryption: Caesar, Vigenère, Hamming Codes
 *   - Math: Valid IPs, Math Expressions, Spiralize Matrix
 *   - Strings: Sanitize Parentheses, Merge Intervals
 * 
 * ARGUMENTS:
...
export async function main(ns) {
    ...
    // Charger historique
    let history = loadHistory(stateMgr, dbg);
    let cycle = 0;
    ...
    while (true) {
        ...
        dbg.clear();
        dbg.header(`BLACKBOX - Cycle ${cycle}`);
        dbg.normal(`${ICONS.TIMER} ${new Date().toLocaleTimeString()}`);
        dbg.separator();
        
        // Scan réseau pour contrats
        dbg.verbose(`${ICONS.NETWORK} Scanning network for contracts...`);
        
        const servers = scanAllServers(ns);
        dbg.verbose(`${ICONS.SUCCESS} Found ${servers.length} servers`);
        ...
        for (const host of servers) {
            ...
            dbg.verbose(`${ICONS.INFO} ${host}: ${contracts.length} contract(s)`);
            
            // Résoudre chaque contrat
            for (const file of contracts) {
                const type = ns.codingcontract.getContractType(file, host);
                const data = ns.codingcontract.getData(file, host);
                
                dbg.verbose(`${ICONS.TARGET} [${host}] ${type}`);
                dbg.ultra(`   Data: ${JSON.stringify(data)}`);
                
                // Tenter résolution
                const solution = solve(type, data);
                
                if (solution !== null) {
                    const reward = ns.codingcontract.attempt(solution, file, host);
                    
                    if (reward) {
                        // ✅ SUCCÈS
                        contractsSolved++;
                        
                        dbg.success(`[${host}] ${type} RÉSOLU !`);
                        dbg.normal(`   ${ICONS.ROCKET} Reward: ${reward}`);
                        
                        ns.tprint(`${ICONS.SUCCESS} [${host}] ${type}: ${reward}`);
                        dbg.toastSuccess(`Contract résolu: ${type}`);
                        ...
                        history.stats.totalSolved++;
                        history.stats.lastSolvedAt = timestamp;
                        
                    } else {
                        // ❌ ÉCHEC
                        contractsFailed++;
                        
                        dbg.warn(`[${host}] ${type} ÉCHEC`);
                        dbg.ultra(`   Solution tentée: ${JSON.stringify(solution)}`);
                        ...
                        history.stats.totalFailed++;
                    }
                } else {
                    // Type non supporté
                    dbg.warn(`[${host}] ${type} - Type non supporté`);
                    
                    history.unsupported.push({
                        timestamp: timestamp,
                        host: host,
                        file: file,
                        type: type
        ...
        dbg.separator();
        dbg.normal(`${ICONS.CHART} Contracts found: ${contractsFound}`);
        if (contractsSolved > 0) {
            dbg.success(`Solved this cycle: ${contractsSolved}`);
        }
        if (contractsFailed > 0) {
            dbg.warn(`Failed this cycle: ${contractsFailed}`);
        }
        
        dbg.separator();
        dbg.normal(`${ICONS.CHART} Total stats:`);
        dbg.normal(`   Solved: ${history.stats.totalSolved}`);
        dbg.normal(`   Failed: ${history.stats.totalFailed}`);
        
        // ══════════════════════════════════════════════════════════════
        // SAVE HISTORY
        // ══════════════════════════════════════════════════════════════
        
        // Trim history (garder 100 derniers de chaque)
        if (history.solved.length > 100) {
            history.solved = history.solved.slice(-100);
        }
        if (history.failed.length > 100) {
            history.failed = history.failed.slice(-100);
        }
        if (history.unsupported.length > 50) {
            history.unsupported = history.unsupported.slice(-50);
        }
        
        await stateMgr.save("contracts-history.json", history);
        dbg.ultra(`${ICONS.SUCCESS} History saved`);
        
        dbg.separator();
        dbg.normal(`${ICONS.TIMER} Next scan in ${SCAN_INTERVAL/1000}s...`);
...
/**
 * Charger historique contracts
 * @param {StateManager} stateMgr
 * @param {Debug} dbg
 * @returns {object}
 */
function loadHistory(stateMgr, dbg) {
    let history = stateMgr.load("contracts-history.json");
    ...
    dbg.verbose(`${ICONS.CHART} History loaded: ${history.stats.totalSolved} solved all-time`);
    return history;
}
...
// ══════════════════════════════════════════════════════════════
// SOLVE FUNCTION (CORE LOGIC - INCHANGÉ)
// ══════════════════════════════════════════════════════════════

/**
 * Résout un coding contract
 * @param {string} type - Type du contrat
 * @param {any} data - Données du contrat
 * @returns {any|null} Solution ou null si type non supporté
 */
